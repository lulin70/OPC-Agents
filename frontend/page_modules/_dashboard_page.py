...
ALL_PANELS_META = [
    ...
    ("financial_summary", _t("dashboard_monthly_finance"), _t("dash_panel_finance_desc")),
    ("activity_timeline", _t("dashboard_activity_timeline"), _t("dash_panel_timeline_desc")),
...
def _render_template_controls(config: DashboardConfig):
    with st.expander(_t("dash_settings_title"), expanded=False):
        ...
        with col_apply:
            if st.button(_t("dash_btn_apply"), type="primary", use_container_width=True):
...
def _render_demo_dashboard():
    ...
    with col1:
        ...
        st.metric(_t("dash_cumulative_income"), f"¥{trend['total']:,}", delta=trend['growth'])
        labels = trend["labels"]
        values = trend["values"]
        import pandas as pd
    ...
    with col2:
        ...
        with fin_col2:
            st.metric(_t("dash_expense_metric"), f"¥{fs['expenses']:,}", delta_color="inverse")
        with fin_col3:
    ...
    with col3:
        ...
        for client in demo["client_health"]:
            ...
            st.markdown(f"**{client['name']}** — {arrow} {_t('dash_score')} {client['score']} | {_t('dash_projects')} {client['projects']}")
    ...
    with col6:
        ...
        for skill in demo["skill_usage"]:
            st.markdown(f"**{skill['name']}** — {_t('dash_calls')} {skill['count']} {_t('dash_calls')}")
...
def _render_income_trend_panel(density: DensityLevel = DensityLevel.STANDARD, full_width: bool = False):
    ...
    if _is_demo_mode():
        ...
        st.line_chart(chart_data.set_index(_t("dash_month_col")), use_container_width=True)
    ...
    if density == DensityLevel.DETAILED:
        ...
        st.markdown(f"**{arrow} {_t('dash_monthly_profit_label').format(val=latest, pct=change)}**")
...
def _render_client_health_panel(density: DensityLevel = DensityLevel.STANDARD, full_width: bool = False):
    ...
    if density == DensityLevel.COMPACT:
        st.markdown(f"**{_t('dash_total_summary').format(t=total, a=active, s=silent_count)}**")
        return
    ...
    if density == DensityLevel.DETAILED and customers:
        ...
        for c in customers[:5]:
            name = c.get("name", "Unknown")
            interactions = c.get("interactions", 0)
            customer_data.append({_t("dash_client_name"): name, _t("dash_interactions"): interactions})
        if customer_data:
            df = pd.DataFrame(customer_data)
            st.dataframe(df, use_container_width=True, hide_index=True)
...
def _render_task_completion_panel(density: DensityLevel = DensityLevel.STANDARD, full_width: bool = False):
    ...
    if _is_demo_mode():
        ...
        if density == DensityLevel.COMPACT:
            st.markdown(f"**{emoji} {_t('dash_completion_rate')} {completion_rate:.1f}%（Demo）** ({done}/{total})")
            return
        ...
        st.progress(completion_rate / 100, text=_t("dash_progress_demo").format(rate=completion_rate))
        import pandas as pd
        status_df = pd.DataFrame([
            {_t("dash_status"): _t("dash_status_done"), _t("dash_count_col"): done, _t("dash_percentage_col2"): f"{done/total*100:.1f}%", "color": "green"},
            {_t("dash_status"): _t("dash_status_progress"), _t("dash_count_col"): in_progress, _t("dash_percentage_col2"): f"{in_progress/total*100:.1f}%", "color": "orange"},
            {_t("dash_status"): _t("dash_status_blocked"), _t("dash_count_col"): blocked, _t("dash_percentage_col2"): f"{blocked/total*100:.1f}%", "color": "red"},
        ])
    ...
    if density == DensityLevel.COMPACT:
        emoji = "🟢" if completion_rate >= 70 else ("🟡" if completion_rate >= 40 else "🔴")
        st.markdown(f"**{emoji} {_t('dash_completion_rate')} {completion_rate:.1f}%** ({completed}/{total})")
        return
    ...
    st.progress(completion_rate / 100, text=_t("dash_progress_text").format(rate=completion_rate))

    if density == DensityLevel.DETAILED and by_status:
        import pandas as pd
        status_df = pd.DataFrame([
            {_t("dash_status"): _t("dash_status_done"), _t("dash_count_col"): completed, _t("dash_percentage_col2"): f"{completed/total*100:.1f}%"},
            {_t("dash_status"): _t("dash_status_progress"), _t("dash_count_col"): in_progress, _t("dash_percentage_col2"): f"{in_progress/total*100:.1f}%" if total > 0 else "0%"},
            {_t("dash_status"): _t("dash_status_pending"), _t("dash_count_col"): pending, _t("dash_percentage_col2"): f"{pending/total*100:.1f}%" if total > 0 else "0%"},
        ])
        st.dataframe(status_df, use_container_width=True, hide_index=True)
...
def _render_financial_summary_panel(density: DensityLevel = DensityLevel.STANDARD, full_width: bool = False):
    ...
    if _is_demo_mode():
        ...
        if density == DensityLevel.COMPACT:
            st.markdown(f"**{_t('dash_income_expense_profit_demo').format(inc=income, exp=expense, net=net_profit)}**")
            return
        col_inc, col_exp, col_profit = st.columns(3)
        with col_inc:
            st.metric(_t("dash_cumulative_income_demo"), f"¥{income:,}")
        with col_exp:
            st.metric(_t("dash_total_expense_demo"), f"¥{expense:,}", delta_color="inverse")
        with col_profit:
            st.metric(_t("dash_net_profit_demo"), f"¥{net_profit:,}", delta=f"¥+{(net_profit - income * 0.75):,}")
        if density != DensityLevel.COMPACT:
            st.caption(_t("dash_estimated_tax").format(tax=tax_est, margin=(net_profit/income*100)))
            import pandas as pd
            comparison = pd.DataFrame({
                _t("dash_category"): [_t("dash_income_metric"), _t("dash_expense_metric"), _t("dash_profit_metric")],
                _t("dash_amount_col"): [income, expense, net_profit],
            })
            st.bar_chart(comparison.set_index(_t("dash_category")), use_container_width=True)
        return
    ...
    if density == DensityLevel.COMPACT:
        st.markdown(f"**{_t('dash_income_expense_profit').format(inc=income, exp=expense, profit=profit)}**")
        return
    ...
    if density == DensityLevel.DETAILED and (income > 0 or expense > 0):
        import pandas as pd
        comparison = pd.DataFrame({
            _t("dash_category"): [_t("dash_income_metric"), _t("dash_expense_metric")],
...
def _render_skill_usage_panel(density: DensityLevel = DensityLevel.STANDARD, full_width: bool = False):
    ...
    if _is_demo_mode():
        ...
        if density == DensityLevel.COMPACT:
            ...
            st.markdown(f"**{_t('dash_total_calls_demo_fmt').format(n=total_calls, name=top_skill['name'], pct=top_skill['usage']/total_calls*100)}**")
            return
        ...
        if density == DensityLevel.DETAILED:
            ...
            st.bar_chart(chart_df.set_index(_t("dash_skill_col")), use_container_width=True, horizontal=True)
        return
    ...
    if density == DensityLevel.COMPACT:
        ...
        st.markdown(f"**{_t('dash_total_calls_fmt').format(n=total_calls, name=top_name, pct=pct)}**")
        return
