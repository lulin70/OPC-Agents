...
class AsyncTaskExecutor:
            ...
            for task in self._tasks.values():
                if task.status in [TaskStatus.PENDING, TaskStatus.RUNNING]:
                    active.append({
                        ...
                        'status': task.status.value,
                        'prompt': task.prompt[:50] + ('...' if len(task.prompt) > 50 else ''),
                        'elapsed': time.time() - task.created_at,
                    })
            return active
    ...
    def _run_worker(self, task_id: str, execute_func: Callable, **kwargs):
        ...
        try:
            ...
            elapsed = (task.completed_at or time.time()) - (task.started_at or task.created_at)
            logger.info(
    ...
    def _cleanup_old_tasks(self):
        ...
        to_remove = set(tid for tid, _ in completed_tasks) - set(tid for tid, _ in keep_recent)
