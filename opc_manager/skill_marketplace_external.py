"""External skill marketplace implementation.

Extracted from ``skill_marketplace`` (facade) to keep each module focused on a
single concern. This module owns the external registry search / install /
uninstall flow, MCP server connections, and sandboxed execution of installed
external skills.

Public names are re-exported by ``opc_manager.skill_marketplace`` for backward
compatibility — existing importers should continue to import from the facade.
"""
...
class ExternalSkillMarketplace:
    ...
    def install_skill(
        ...
        record_id = gen_id()
        execute_write(
            "INSERT INTO external_skills (id, name, description, source, "
            "version, skill_config, trust_level, installed_at) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (
                record_id,
    ...
    def _validate_skill_package(self, package_info: Dict[str, Any]) -> bool:
        ...
        for field_name in required_fields:
            if not package_info.get(field_name):
                logger.warning("技能包缺少必填字段: %s", field_name)
                return False
    ...
    def execute_in_sandbox(
        ...
        if not self._validate_entry_point(entry_point):
            return {
                "success": False,
                "error": (
                    f"entry_point 路径不合法，仅允许 data/custom_skills/、"
                    f"data/marketplace/external_skills/、plugins/ "
                    f"目录下的文件: {entry_point}"
                ),
            }
