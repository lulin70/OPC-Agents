...
class SearchCache:
    ...
    def __init__(self, max_size: int = SEARCH_CACHE_MAX_SIZE, ttl: int = SEARCH_CACHE_TTL_SECONDS):
    ...
    def set(self, query: str, max_results: int, results: List[Dict]):
        ...
        logger.info(f"[SearchCache] 写入: {query[:30]}... (缓存大小:{len(self._cache)})")
...
class TaskEngineV3:
    ...
    def execute(self, user_input: str, session_ctx: 'SessionContextManager' = None) -> TaskResult:
        ...
        if session_ctx and session_ctx.get_turn_count() > 0:
            ...
            if history_context:
                ...
                logger.info(f"[TaskEngineV3] 已注入{session_ctx.get_turn_count()}轮上下文")

        try:
            task_type, confidence = IntentClassifier.classify(enriched_input)
            logger.info(f"[TaskEngineV3] 意图: {task_type.value} (置信度:{confidence:.2f}, 输入长度:{len(enriched_input)})")
            ...
            if cache_stats['hits'] + cache_stats['misses'] > 0:
                logger.info(f"[TaskEngineV3] 搜索缓存统计: 命中{cache_stats['hits']}/{cache_stats['hits']+cache_stats['misses']}")
            return result
    ...
    def _search(self, query: str, max_results: int = 8) -> Tuple[List[Dict], List[Dict]]:
        ...
        if cached is not None:
            sources = [{"title": r.get("title", ""), "url": r.get("href", "")} for r in cached if r.get("href")]
        ...
        try:
            ...
            try:
                ...
                if processed.fallback_used:
                    logger.info(f"[TaskEngineV3] 搜索'{query[:30]}...'使用知识库兜底({len(results)}条)")
                ...
                logger.warning(f"[TaskEngineV3] SearchResultProcessor处理失败(使用原始结果): {proc_error}")
                results = raw_results
    ...
    def _gen_real_report(self, query: str, context: List[str], search_results: List[Dict]) -> str:
        ...
        lines.append(f"|------|---------|----------|--------|\n")
        lines.append(f"| 效率指标 | 建议建立基线后持续追踪 | 行业前25%水平 | 持续改进 |\n")
        lines.append(f"| 质量指标 | 建议建立基线后持续追踪 | 客户满意度≥4.5/5 | 缺陷密度<0.5/KLOC |\n")
        lines.append(f"| 成本指标 | 建议建立基线后持续追踪 | 控制在预算±10%内 | ROI>1.5 |\n\n")
        ...
        lines.append(f"> 建议将此报告作为工作基础，结合实际情况填充具体数据和责任人。\n")
    ...
    def _gen_real_plan(self, query: str, context: List[str], search_results: List[Dict]) -> str:
        ...
        lines.append(f"> 方案中的时间节点、资源分配和风险应对措施均为基于行业标准给出的具体建议，可直接作为工作启动依据。\n")
