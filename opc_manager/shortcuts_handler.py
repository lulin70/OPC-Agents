"""
Apple Shortcuts Integration Handler for OPC-Agents v0.2.0
...
- 保持简洁专业"""
            ...
            if response and response.strip():
                return ShortcutResult(True, f"任务完成:\n{response[:500]}", {
                    "action": "quick_task",
    ...
    def query_status(self) -> ShortcutResult:
        ...
        try:
            ...
            return ShortcutResult(True, status_msg, {
                "date": today,
                "tasks": t_count,
                "income": float(i_total)
            })
            
        except Exception as e:
            return ShortcutResult(False, f"查询失败: {str(e)}")
    
    def create_deliverable(self, title: str, dtype: str = "document", 
                           content: str = "") -> ShortcutResult:
        """
        ...
        try:
            ...
            type_labels = {"document": "📄文档", "report": "📊报告", 
                          "proposal": "📝方案", "invoice": "🧾发票"}
            label = type_labels.get(dtype, "📁文件")
            
            return ShortcutResult(True, f"{label}已创建: {title}\nID: {did[:8]}", {
                "id": did,
                "title": title,
                "type": dtype
            })
    ...
    def record_income(self, amount: float, client: str = "", 
                     source: str = "") -> ShortcutResult:
        """
        ...
        try:
            ...
            execute_write(
                ...
                (iid, "income", amount, source or "其他", source or "其他", today, client or "未知客户", now)
            )
            
            return ShortcutResult(True, f"💰 收入已记录\n金额: ¥{amount:,.0f}\n客户: {client or '未知'}\n来源: {source or '其他'}", {
                "id": iid,
                "amount": amount,
                "client": client,
                "date": today
    ...
    def daily_report(self) -> ShortcutResult:
        ...
        try:
            ...
            tasks = execute_query("SELECT * FROM tasks WHERE DATE(created_at)=? ORDER BY created_at DESC LIMIT 10", (today,))
            income_rows = execute_query("SELECT * FROM finance_records WHERE type='income' AND DATE(date)=? ORDER BY date DESC", (today,))
            deliverable_tasks = execute_query("SELECT * FROM tasks WHERE tags LIKE 'deliverable:%' AND DATE(created_at)=? ORDER BY created_at DESC LIMIT 5", (today,))
            ...
            if deliverable_tasks:
                for d in deliverable_tasks[:5]:
                    tag = d.get('tags', '')
                    dtype = tag.replace('deliverable:', '') if tag.startswith('deliverable:') else '?'
            ...
            if tasks:
                for t in tasks[:5]:
                    ...
                    report += f"   {status} {t.get('title', t.get('description', ''))[:40]}\n"
            
            report += f"\n{'='*30}\n由 OPC-Agents 自动生成 🤖"
            
            return ShortcutResult(True, report, {
                "date": today,
...
Examples:
  ...
  python -m opc_manager.shortcuts_handler record_income --amount 5000 --client "张三"
    ...
    p3.add_argument("--type", default="document", help="Type: document/report/proposal/invoice")
    ...
    actions = {
        ...
        "create_deliverable": lambda: handler.create_deliverable(args.title, args.type, args.content),
        "record_income": lambda: handler.record_income(args.amount, args.client, args.source),
