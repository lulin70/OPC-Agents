...
class WeChatGateway:
    ...
    def set_message_handler(self, handler: Callable[[WeChatMessage], Awaitable[WeChatResponse]]):
    ...
    async def handle_callback(self, query_params: Dict[str, str], body: str) -> str:
        ...
        if msg.msg_type == WeChatMsgType.EVENT and msg.event_type in ("subscribe", "unsubscribe"):
            return "success"
    ...
    @staticmethod
    def build_confirmation_card(title: str, params: dict, 
                                   confirm_text="确认", cancel_text="取消") -> str:
        lines = [f"📋 {title}", ""]
