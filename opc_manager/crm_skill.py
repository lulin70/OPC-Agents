...
def add_customer(
    ...
    if (
        phone
        and not re.match(r"^1[3-9]\d{9}$", phone)
        and not re.match(r"^\+\d{7,15}$", phone)
    ):
    ...
    try:
        ...
        return {
            ...
            "id": customer_id,
            "message": (
                f"客户已录入: {name} ({company})" if company else f"客户已录入: {name}"
            ),
        }
...
def get_customer(customer_id: str = "", name: str = "") -> Dict[str, Any]:
    ...
    deals = execute_query(
        "SELECT * FROM deals WHERE customer_id=? ORDER BY date DESC", (customer["id"],)
    )
...
def search_customers(
    company: str = "", tags: str = "", status: str = "", source: str = ""
) -> Dict[str, Any]:
    conditions = []
    params: list = []
    if company:
...
def add_deal(
    ...
    try:
        ...
        if status == "closed_won":
            ...
            stmts.append(
                ("UPDATE customers SET last_contact=? WHERE id=?", (now, customer_id)),
            )
        ...
        AuditLogger.log(
            "crm_deal_added",
            {"id": deal_id, "customer_id": customer_id, "amount": amount},
        )
        return {
            "success": True,
            "id": deal_id,
            "message": f"合作记录已添加: {description}",
        }
    except Exception as e:
        return {"success": False, "error": str(e)}
...
def add_follow_up(
    customer_id: str, content: str, follow_date: str = ""
) -> Dict[str, Any]:
    ...
    try:
        ...
        AuditLogger.log(
            "crm_follow_up_added", {"id": follow_up_id, "customer_id": customer_id}
        )
        return {
            "success": True,
            "id": follow_up_id,
            "message": f"跟进记录已添加: {content[:30]}",
        }
    except Exception as e:
        return {"success": False, "error": str(e)}
