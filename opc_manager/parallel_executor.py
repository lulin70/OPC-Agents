...
class ParallelExecutor:
    ...
    async def execute_parallel(
        ...
        speedup_factor = self._calculate_speedup(total_time_ms, total_serial_time_ms, task_count)
    ...
    async def _execute_single_task(
        ...
        for attempt in range(retries_remaining + 1):
            try:
                ...
                execution_time_ms = (time.time() - (task_start if 'task_start' in locals() else time.time())) * 1000
        ...
        failure_result = TaskResult(
            ...
            execution_time_ms=execution_time_ms if 'execution_time_ms' in locals() else 0,
            task_index=task_index,
    ...
    def _merge_results(
        ...
        if not successful_results:
            ...
            return f"\n\n❌ 所有并行任务均失败:\n" + "\n".join(f"- {e}" for e in failed_errors if e)
