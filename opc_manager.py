...
class OPCManager:
    ...
    def trae_request(self, prompt: str, agent_type: str, model_name: str = None) -> Optional[str]:
        """Send request to AI model API or use local simulation
        ...
        print(f"[Local Simulation] Processing request for {agent_type} using {model_name}...")
...
The campaign will run for 4 weeks with a budget allocation across different channels.''',
            'default': f"This is a simulated response from the {agent_type} agent using {model_name} model. In a real implementation, this would be generated by the {model_name} API based on the prompt: {prompt[:100]}..."
        }
        
        return agent_responses.get(agent_type, agent_responses['default'])
    
    def assign_task(self, task: str, department: str, agent: Optional[str] = None, model: Optional[str] = None) -> Optional[str]:
...
Please provide a detailed response addressing this task."""
        ...
        # Send to AI model
        model_name = model or self.config.get('models', {}).get('default', 'trae')
        print(f"正在处理任务: {task[:50]}...")
        print(f"分配给: {agent_type}")
        print(f"使用模型: {model_name}")
        return self.trae_request(prompt, agent_type, model_name)
