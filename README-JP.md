...
**⚡ コアフロー**:
```
朝のサマリー → 沈黙顧客 → フォローアップ → メール/タスク納品 → 結果を記録
```

**🎯 オペレーター業務フロー**：毎朝サマリーを確認し、沈黙顧客を特定、フォローアップを作成・確認してメールまたはタスクで届け、結果を顧客ライフサイクルへ記録します。調査、計画、財務などの成果物も引き続き直接作成できます。
...
- **📊 継続的な品質向上**: 4970テスト、カバレッジ74%+、全CIゲート通過（ruff/mypy/Black/E2E/coverage/radon D+/Bandit/pip-audit/Docker build/バージョン一致性/三言語README一致性）。D02成熟度評価82点 B+。
...
│       ├── word_exporter.py
│       └── image_exporter.py
├── tests/                 # テストスイート（138テストファイル、4970テスト、100%合格）
├── docs/                  # プロジェクトドキュメント
│   ├── API.md             # APIドキュメント
