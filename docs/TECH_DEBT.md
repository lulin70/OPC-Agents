...
> **最后更新**：v0.5.9（2026-08-01）— TD-005 校准 + 三贤者/CarryMem E2E 补充
...
| 状态 | 数量 | 说明 |
|------|------|------|
| ✅ 已解决 | 5 | v0.5.7 修复 4 项 + v0.5.9 校准 TD-005 已解决 |
| 🔄 进行中 | 2 | v0.5.8 推进（TD-006/007） |
| ⏳ 待处理 | 5 | v0.6.0+ 规划 |
| **合计** | **12** | |
...
- **状态**：✅ 已解决（v0.5.9 校准 — 实测全部通过）
- **来源**：ASSESSMENT_v0.5.6 P1-1
- **原问题描述**：E2E 测试 13 个真实失败
  - 7 asyncio 事件循环冲突（`asyncio.run()` in worker threads）
  - 3 morandi 主题缺陷
  - 1 过期断言（主题数 5→7）
  - 2 a11y 对比度不达标（Demo 横幅白底白字）
- **校准结果**（2026-08-01 实测）：
  - 7 asyncio 冲突：已在 v0.5.7→v0.5.9 间修复（`_run_async` 子线程模式，test_e2e_user_journeys.py 等 58 测试全通过）
  - 3 morandi 主题缺陷：已修复（test_theme_dark.py 3 测试全通过）
  - 1 过期断言：已修复（test_docker_deployment.py `test_all_themes_exist` 已更新为 7 主题）
  - 2 a11y 对比度：已修复（test_a11y_all_themes.py 32 测试 + test_a11y_axe.py 3 测试全通过）
- **实测验证**：208 个 E2E 测试全通过（68 静态 + 58 集成 + 37 apptest/api + 45 主题/a11y/视觉回归）
- **教训**：TECH_DEBT.md 状态滞后 2 版本（v0.5.7→v0.5.9），符合 project_memory 教训"基于过期描述的任务需先校验前提"
- **补充**：v0.5.9 新增 70 个 E2E 测试填补架构核心空白：
  - `test_parallel_sages_e2e.py`（38 测试）：三贤者并行投票/fail-close/关键决策点保护/三路路由/决策日志持久化
  - `test_memory_bridge_e2e.py`（32 测试）：CarryMem 跨会话记忆/规则引擎/飞轮成长/失败经验/数据可携带性
...
| v0.5.9 | — | TD-005 | 校准 TD-005（13 个 E2E 失败已于 v0.5.7→v0.5.9 间修复）+ 新增 70 个 E2E 测试 |
| v0.6.0+ | — | TD-006~012 | 计划解决剩余 7 项 |
