...
> **最后更新**: 2026-07-12（DevSquad 共识推进 P3-4 Mock 反模式修复第三批完成，v0.3.17 发布） | **版本**: v0.3.17 (Beta) | **许可**: MIT
>
...
- ~~Mock 反模式修复（P3-4）~~ ⏳ 进行中 (2026-07-12, v0.3.17 第三批): 7 文件修复完成（第一批 3 文件: test_email_skill_coverage Mock文件系统→tmp_path+monkeypatch / test_simple_llm_service Mock os.environ.get→monkeypatch.setenv/delenv / test_executor_opinion MagicMock→RaisingLLMService真实fake类；第二批 2 文件: test_delta_integration 5处 MagicMock LLM→MockLLMService/RaisingLLMService / test_integration_modules 12处 MagicMock SkillRegistry/Skill→FakeSkillRegistry/FakeSkill + 真实 SkillRegistry 实例；第三批 2 文件: test_undo_panel 17处@patch+22处MagicMock→真实UndoManager实例+monkeypatch注入 / test_skill_executors 30+处MagicMock→8个真实fake类），剩余 1 文件待推进（test_timeline_view ~16处非streamlit Mock: UndoRecord/AuditLog/ProgressEmitter MagicMock，~30处streamlit Mock合理保留）
