...
- **🔧 v0.4.0 发布前质量巩固**: bandit B608 安全告警清零（5 处 nosec 注释，列名白名单 + 值参数化安全模式）；T7 Mock 反模式系列正式关闭（累计 5 文件 42 处替换，剩余 56 文件 532 处为必要 Mock）；tool_system.py Facade 拆分确认（222 行 Facade + 5 子模块，覆盖率 84-100%）；3 大文件 SRP 评估完成（data_manager/task_engine_v3_executors/task_orchestrator 均非 God Class）；Mock 分类判定标准文档化（7 类 Mock + 自检清单）；D05 E2E 199/200 通过（99.5%，1 环境失败：Ollama 未启动）。详见 [RELEASE_NOTES_v0.4.0.md](docs/releases/RELEASE_NOTES_v0.4.0.md)。

## 🆕 v0.3.36 Highlights

- **🔧 v0.3.36 T7 第 2 批 Mock 精准替换 + T7 系列关闭**: 深度扫描 Top 5 候选文件，实际替换 6 处（原估计 ~45 处，-87%）；T7.6 test_consensus_engine 替换 0 处（77 处 @patch.object 全部是测试隔离必要 Mock，避免 SQLite DB 副作用）；T7.7 test_memory_bridge 替换 6 处（局部 MagicMock → FakeRuleMatch/FakeSuggestion 类，消除重复配置代码）；T7.8 评估后跳过 3 文件（subprocess/streamlit/已有 Fake 类均为必要 Mock）；T7 第 3 批正式关闭（剩余 56 文件 532 处为必要 Mock）。T7 系列总替换 42 处（v0.3.35: 36 + v0.3.36: 6）。4164 测试通过 + 0 失败。详见 [ROADMAP_v0.3.36.md](docs/ROADMAP_v0.3.36.md)。
...
- **📊 质量持续提升**：4621 个测试，覆盖率 74%+，CI 全门禁通过（ruff/mypy/Black/E2E/coverage/radon D+/Bandit/pip-audit/Docker build/版本一致性/三语 README 一致性）。D02 成熟度评估 82 分 B+。
...
- **🔧 v0.3.29 修复 CI 全红问题**：ruff 版本统一 0.15.21（CI 0.6.9→0.15.21）+ README 测试数修正（4278→4193）。v0.4.0 发布待 D04 重新评估。
...
| **0.3.22** | **2026-07-12** | **P3-5 Mock 反模式修复** — 2 个测试文件 MagicMock 替换为真实 fake 类（test_brain_modules 40处→6个fake类: FakeLLMService/FakeSkill/FakeAsyncSkill/FakeSkillRegistry/FakeTaskResult/FakeTaskEngine / test_live_log_panel 4处→2个fake类: FakeAuditLog/FakeProgressEmitter），保留合理 @patch 和 psutil Mock |
...
| **0.3.11** | **2026-07-11** | **radon cc D+ blocking 门禁(P3-2)** — 6 个 D/E 级函数全部降级（`_parse_analysis_result` E(36)→A(2) / `finance_skill.execute_goal` D(30)→A(4) / `_extract_keywords` D(29)→A(2) / `_calculate_quality_score` D(28)→B(6) / `_parallel_data_analysis` D(22)→A(4) / `_execute_collaborative` D(21)→A(4)），CI radon cc 从 non-blocking 转 D+ blocking，Dockerfile/start.sh 版本号修复（0.3.5→0.3.11） |
