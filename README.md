...
一个为一人公司（OPC）设计的多代理系统，灵感来自GitHub项目Agency-Agents，具有增强的AI能力。

## 概述

该项目实现了一个虚拟代理机构，包含多个专业的AI代理，可以处理各种任务。系统具有总裁办智能任务处理链、三贤者决策系统、网页搜索、MCP GitHub集成、财务监控等能力。

## 功能

- **36个专业部门**，覆盖设计/开发/营销/财务/运营等领域
- **180+专业代理**（来自官方Agency-Agents项目 + A2A协议）
- **多模型支持**：GLM、OpenAI、Anthropic、Google、Azure、本地模型
- **总裁办智能任务处理链**：闲聊→搜索→任务三模式，自动意图判断
- **三贤者决策系统**：战略/执行/创新三视角战略决策
- **网页搜索**：DuckDuckGo免费搜索，辅助决策和信息获取
- **MCP GitHub集成**：搜索/获取/导入外部Agent和Skill
- **任务管理**：创建/重命名/删除，自动创建工作目录
- **任务工作目录**：按`YYYYMMDD_HHMM_任务名`命名，支持Finder打开
- **财务部**：Token监控、消费报告、预算告警
- **系统监控**：CPU/内存/磁盘、组件状态、任务统计、告警
