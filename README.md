...
- **TaskScheduler抽象层**：统一调度接口（schedule/cancel/pause/resume），支持多种调度策略
- **步骤自动重试**：失败自动重试（可配置次数），重试耗尽才标记failed
...
- **任务事件**：task_completed/task_failed事件，HR模块自动响应

**配置管理**
- **配置热更新**：监控config.toml变化，自动重载配置，无需重启服务
