...
class TestConcurrencyControl:
    ...
    async def test_concurrency_one(self):
        ...
        tasks = [
            TaskSpec(func=make_ordered_task(i), description=f"Task {i}")
            for i in range(3)
        ]
...
class TestMergeStrategies:
    ...
    async def test_concat_strategy(self):
        ...
        result = await executor.execute_parallel(
            tasks, merge_strategy=MergeStrategy.CONCAT
        )
    ...
    async def test_deduplicate_strategy(self):
        ...
        result = await executor.execute_parallel(
            tasks, merge_strategy=MergeStrategy.DEDUPLICATE
        )
    ...
    async def test_first_success_strategy(self):
        ...
        result = await executor.execute_parallel(
            tasks, merge_strategy=MergeStrategy.FIRST_SUCCESS
        )
    ...
    async def test_merge_with_failures(self):
        ...
        assert (
            len(result.merged_content) > 0
        )  # Should have some content even with failures
...
class TestSpeedEstimation:
    ...
    def test_estimate_speedup_multiple_fast_tasks(self):
        ...
        assert estimate["recommendation"] in [
            "not_worthwhile",
            "recommended",
            "highly_recommended",
        ]
    ...
    def test_speedup_calculation_in_real_execution(self):
        ...
        tasks = [
            TaskSpec(func=moderate_task, description=f"Task {i}") for i in range(4)
        ]
...
class TestShouldParallelize:
    ...
    def test_content_generation_with_document_keywords(self):
        ...
        prompt = "帮我写一份Q2营销报告"
        assert (
            self.engine._should_parallelize(prompt, TaskType.CONTENT_GENERATION) is True
        )
