...
def _get_bare_session_state_accesses(source):
    ...
    for i, line in enumerate(source.split('\n'), 1):
        ...
        for var_name in matches:
            safe_patterns = ['.get(', ' in ', ", '= in", '#', 'if ', 'elif ',
                           'for ', 'while ', 'with ', 'pop(']
...
def test_no_dangerous_bare_session_access():
    ...
    dangerous = [(ln, var, ctx) for ln, var, ctx in violations
                  if '=' not in ctx and 'if ' not in ctx and 'elif ' not in ctx
                  and 'pop(' not in ctx and 'del ' not in ctx]

    assert len(dangerous) < 20, \
        f"Too many unsafe st.session_state accesses ({len(dangerous)}):\n" + \
        "\n".join(f"  L{ln}: {var} -> {ctx}" for ln, var, ctx in dangerous[:10])
