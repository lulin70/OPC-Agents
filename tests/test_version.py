...
def test_version_in_requirements():
    ...
    assert __version__ in content, \
        f"requirements.txt 中未找到版本号 {__version__}"
...
def test_version_info_matches_version_string():
    """测试version_info与version字符串匹配"""
    from opc_manager.version import __version__, __version_info__
    
    # 从version_info重建版本字符串
    reconstructed = f"{__version_info__[0]}.{__version_info__[1]}.{__version_info__[2]}"
    
    assert reconstructed == __version__, \
        f"version_info({__version_info__}) 与 version({__version__}) 不匹配"
