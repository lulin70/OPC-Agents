...
def test_all_page_routes_defined():
    ...
    required_routes = ['chat', 'deliverables', 'dashboard', 'growth', 'marketplace', 'settings']

    has_new_routing = 'navigate' in source and 'PageKey' in source and 'page_key_map' in source
    if has_new_routing:
        for route in required_routes:
            assert f'PageKey.{route.upper()}' in source or f'"{route}"' in source, \
                f"Missing page route key: {route}"
        return

    missing = [r for r in required_routes if f'page == "{r}"' not in source and f"page == '{r}'" not in source]
...
def test_i18n_import_present():
    ...
    with open(APP_PY) as f:
        source = f.read()
    assert 'from opc_manager.i18n import' in source or 'import opc_manager.i18n' in source, \
        "app.py doesn't import i18n system - translation won't work!"


@pytest.mark.parametrize("component_file", [
