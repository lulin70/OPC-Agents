...
def test_page_modules_directory_exists():
    ...
    assert os.path.isdir(page_modules), "frontend/page_modules/ directory missing!"
...
@pytest.mark.xfail(
    reason="Known F03: dashboard_page has circular import from app.py - will be fixed in Phase 1 structural extraction"
)
def test_no_circular_import_risk():
    ...
    with open(dp) as f:
        content = f.read()
    assert (
        "from frontend.app import" not in content
        and "import frontend.app" not in content
