...
class TestAgentConfig:
    ...
    def test_invalid_agent_id(self):
        ...
        for invalid_id in invalid_ids:
            with pytest.raises(ValidationError):
                AgentConfig(agent_id=invalid_id, display_name="测试")

    def test_too_many_expertise_tags(self):
        """测试过多的专业标签"""
        tags = [f"tag{i}" for i in range(21)]
        with pytest.raises(ValidationError):
            AgentConfig(agent_id="test", display_name="测试", expertise_tags=tags)
...
class TestFileUpload:
    ...
    def test_path_traversal_protection(self):
        ...
        for dangerous in dangerous_filenames:
            upload = FileUpload(
                filename=dangerous, content_type="text/plain", size_bytes=1000
            )
            # 应该移除路径分隔符
...
class TestValidateJsonStructure:
    ...
    def test_valid_nested_json(self):
        """测试有效的嵌套JSON"""
        data = {"level1": {"level2": {"level3": {"value": "test"}}}}
...
class TestIntegration:
    ...
    def test_security_chain(self):
        ...
        upload = FileUpload(
            filename=path_traversal, content_type="text/plain", size_bytes=1000
        )
        # 路径分隔符应该被移除
        assert ".." not in upload.filename
