...
async def run_benchmark_2():
    ...
    tasks_funcs = [
        analysis_dim_1, analysis_dim_2, analysis_dim_3,
        analysis_dim_4, analysis_dim_5
    ]
...
async def run_benchmark_3():
    ...
    tasks_funcs = [
        slow_search, medium_analysis_1, medium_analysis_2,
        quick_lookup_1, quick_lookup_2
    ]
...
async def run_benchmark_4():
    ...
    for max_concurrent in [1, 2, 3, 6]:
        ...
        task_specs = [TaskSpec(func=standard_task, description=f"Task {i}") for i in range(6)]
        ...
        print(f"    Time: {parallel_time*1000:.1f}ms, Speedup: {speedup:.2f}x ({improvement:.1f}% faster)")
    ...
    print("  ✅ PASS: Higher concurrency shows better speedup (with diminishing returns)")
...
async def run_benchmark_5():
    ...
    for task_count in [1, 2, 3, 5]:
        task_specs = [TaskSpec(func=trivial_task, description=f"Task {i}") for i in range(task_count)]
        ...
        assert elapsed < 50, f"Overhead too high: {elapsed:.2f}ms for {task_count} trivial tasks"
        ...
        for name, result_data in [
            ...
            if result_data and len(result_data) >= 3:
                ...
                if serial > 0:
                    ...
                    print(f"  {name:20s}: {improvement:6.1f}% faster (speedup: {speedup:.2f}x)")
