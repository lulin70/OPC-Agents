...
class TestStatusPolling(unittest.TestCase):
    ...
    def test_failed_status_contains_error(self):
        """failed/retrying状态应包含错误信息"""

        def failing_task(prompt, cancel_event, **kwargs):
            raise ValueError("模拟执行失败")

        executor_no_retry = AsyncTaskExecutor(max_retries=0)
        task_id = executor_no_retry.submit("失败任务", execute_func=failing_task)
        time.sleep(0.2)

        status = executor_no_retry.get_status(task_id)
    ...
    def test_retry_on_failure(self):
        ...
        def retry_task(prompt, cancel_event, **kwargs):
            nonlocal call_count
            call_count += 1
            if call_count < 3:
                raise ValueError(f"第{call_count}次失败")
            return {"content": "重试成功", "success": True}

        executor_retry = AsyncTaskExecutor(max_retries=2, retry_backoff_base=0.1)
        task_id = executor_retry.submit("重试任务", execute_func=retry_task)
        time.sleep(3.0)

        status = executor_retry.get_status(task_id)
        self.assertEqual(status["status"], "done")
        self.assertEqual(status["result_content"], "重试成功")
