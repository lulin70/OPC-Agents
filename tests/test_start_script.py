...
class TestStartScript:
    ...
    def test_start_sh_is_executable(self):
        ...
        assert mode & stat.S_IXUSR, "start.sh is not executable (user execute bit missing)"

    def test_start_sh_contains_version(self):
        """start.sh must contain v0.2.0 version marker."""
        path = os.path.join(PROJECT_ROOT, "start.sh")
        with open(path, "r") as f:
            content = f.read()
        assert "v0.2.0" in content, "start.sh missing v0.2.0 version string"
    ...
    def test_start_sh_auto_installs_deps(self):
        ...
        with open(path, "r") as f:
            content = f.read()
        assert "requirements.txt" in content, "start.sh missing requirements.txt install step"
...
class TestRequirementsTxt:
    ...
    def test_requirements_has_layered_comments(self):
        ...
        with open(path, "r") as f:
            content = f.read()
        assert "Core" in content or "REQUIRED" in content, "requirements.txt missing Core/REQUIRED section"
    ...
    def test_requirements_contains_cryptography(self):
        ...
        with open(path, "r") as f:
            content = f.read()
        assert "cryptography" in content, "requirements.txt missing cryptography dependency"
...
class TestDemoMode:
    ...
    def test_demo_data_returns_expected_structure(self):
        ...
        expected_keys = {
            ...
            "financial_summary", "timeline", "skill_usage",
        }
...
class TestFinanceSkillBugFix:
    ...
    def test_no_wrong_table_name_finances(self):
        ...
        wrong_refs = [
            ...
            if "finances" in line.lower() and "finance_records" not in line
    ...
    def test_uses_correct_table_finance_records(self):
        ...
        with open(path, "r") as f:
            content = f.read()
        assert "DELETE FROM finance_records" in content, \
    ...
    def test_undo_functions_check_return_value(self):
        ...
        with open(path, "r") as f:
            content = f.read()
        assert "rows = execute_write" in content, \
            "Undo functions don't capture execute_write return value"
        assert 'rows > 0' in content or "rows == 0" in content, \
